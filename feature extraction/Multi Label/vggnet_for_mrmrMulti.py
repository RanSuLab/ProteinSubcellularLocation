...
def load_data_list():
  ...
  #  dir = '/dev/sda3/linghe/linghe/Multilabel/tmp/'
...
def VggNet(input_op, keep_prob, training):
    ...
    # logits = fc_op(fc7_drop, name="fc8", n_out=classes, regularizer=False)
    logits = tf.layers.dense(fc7_drop,classes,activation=tf.nn.sigmoid)
...
def train(X_train_list, Y_train_list, X_test_list, Y_test_list):
    ...
    #        train_step = tf.train.AdamOptimizer(learning_rate, 0.9).minimize(cross_entropy, global_step=global_step)
...
# train
#evaluate(X_train_list, Y_train_list,X_test_list, Y_test_list, True,'/dev/sda3/linghe/linghe/Multilabel/code/~/model_saving/VggNet/2019-12-25_19.05/proteinModel-32')
