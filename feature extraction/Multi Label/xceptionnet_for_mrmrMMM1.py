...
model_path_last = '~/model_saving/xceptionNetsigmoid/' + date + '_' + hour + '.' + min
...
def next_batch(xs, ys, batch_size, start, size):
    ...
    batch_ys = np.ndarray(shape=(len(batch_ys_list),9),dtype = int,buffer = np.array(getArray(batch_ys_list)),offset=0,order = "C")
    return batch_xs, batch_ys


def xception(inputs,
             num_classes=1001,keep_prob = 1.0,
             is_training=True,
             scope='xception'):
    '''
    The Xception Model!
    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1), \
           ...
           # net = slim.conv2d(net, 2048, [1, 1], scope='block15_conv1')
            #logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=tf.sigmoid, scope='block15_conv2')
           # logits = slim.fully_connected(net, num_classes, activation_fn=tf.sigmoid, scope='fc')
           ...
           # end_points['Logits'] = logits
            ...
            #fc_drop = tf.nn.dropout(fc_128, keep_prob, name="fc_drop")
...
#         x.append(tmp)
#     return x
# def isTrue(list1,list2):
#     result = []
#     for i in range(len(list1)):
#         if(len(list1[i])!=len(list2[i])):
#             result.append([False])
#         else:
#             result.append([np.array_equal(list1[i],list2[i])])
#     return result
def train(X_train_list, Y_train_list, lr_value):
    ...
    fc2, logits = xception(features, classes,keep_prob, True)
   # cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=labels))
    cross_entropy = tf.reduce_mean( tf.keras.losses.binary_crossentropy(labels, logits))
        ...
        for it in range(epochs):
                ...
                for i in range(int(train_size / batch_size)):
                    ...
                    if (i + 1) % 1 == 0:
                        train_cost = sess.run(cross_entropy, feed_dict={features: batch_xs,
                                                                        labels: batch_ys, keep_prob: keep_prob_value,
                                                                        training: True})
                        now_time = str(datetime.datetime.now())
            ...
            if test_size % test_batch_size == 0:
                for x in range(int(test_size / test_batch_size)):
                    ...
                    # print(y_pre)
                    # correct_prediction = isTrue(getLabel(y_pre), getLabel(batch_test_ys))
...
def evaluate(X_test_list, Y_test_list, model_path):
    ...
    fc2, logits = xception(x, classes, False)
    ...
    #with tf.Session() as sess:    
        ...
        if train_size % train_batch_size == 0:
            ...
            for i in range(int(train_size / batch_size)):
                ...
                print(i, '/', int(train_size / batch_size),patch_right_num)
            ...
            book.save('./xceptionnet_featureNew1.xls')
...
#evaluate(X_train_list, Y_train_list,'/dev/sda3/linghe/linghe/Multilabel/code/~/model_saving/xceptionNetsigmoid/2019-12-27_08.06/proteinModel-21')
