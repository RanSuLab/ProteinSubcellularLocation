...
def AlexNet(input_op,keep_prob,training):
    ...
    logits = out = tf.layers.dense(fc2_drop,classes,activation=tf.nn.sigmoid)
...
def train(X_train_list,Y_train_list,lr_value):
    ...
    learning_rate = tf.train.exponential_decay(lr_value, global_step, 10000, 0.96, staircase=False)
    with tf.name_scope('optimizer'):
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
           # train_step = tf.train.AdadeltaOptimizer0.9).minimize(cross_entropy, global_step=global_step)
...
# './model_saving/resnet_softmax/2018-08-11_00.35/osteoporosis_classifier'  self.model_save_path
        ...
        if train_size % train_batch_size == 0:
            ...
            for i in range(int(train_size / train_batch_size)):
                ...
                batch_xs, batch_ys = next_batch(X_test_list, Y_test_list, train_batch_size, i, train_size)
                ...
                print(i, '/', int(train_size / train_batch_size), '   ', patch_right_num)
           ...
           # book.save('./alexnet_featureNew1.xls')
...
evaluate(X_train_list, Y_train_list,'/dev/sda3/linghe/linghe/Multilabel/code/~/model_saving/AlexNet/2019-12-25_19.05/proteinModel-32')
...
#    for i in range(len(Y_test_list)):
    print(Y_test_list)
#readLabel(X_train_list,X_train_list)
